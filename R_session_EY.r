y<-1:5
plot(x=rep(x,length(z)),y=z)
temp<-data.frame (x,y,z)
# 
dfd<-effects::TitanicSurvival # From the effect packaga, take the titanic dataset
View(dfd)
summary(dfd)
?mean
mean(dfd$age, na.rm=TRUE)
rm(list=setdiff(ls()),"dfd") # removing everything except the my dfd dataset
require(RMySQL)  
#################################
...
dfd$y <- dfd$x^3 + 100*runif(nrow(dfd)) 
#Get y based upon x and some noise.
